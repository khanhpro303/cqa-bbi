...
> (vd người dùng gõ lại tên dòng, hoặc mã `SP\d{6}`) hoặc khi pending đã hết hạn. Trong các
...
- Whenever you re-query `resource="products"` for a web name ALREADY picked from a list, ALWAYS pass `exact_web_name=true` so the option list is not pushed again.
