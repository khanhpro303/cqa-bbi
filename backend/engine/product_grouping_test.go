...
func TestBuildNumberedWebNameList(t *testing.T) {
	tests := []struct {
		name   string
		groups []WebGroupMatch
		want   string
	}{
		{"empty", nil, ""},
		{"empty slice", []WebGroupMatch{}, ""},
		{"single", []WebGroupMatch{{WebName: "FF901"}}, "1. FF901"},
		{
			name: "preserves input order",
			groups: []WebGroupMatch{
				{WebName: "FF901"},
				{WebName: "FF901 Carbon"},
			},
			want: "1. FF901\n2. FF901 Carbon",
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if got := BuildNumberedWebNameList(tc.groups); got != tc.want {
				t.Errorf("BuildNumberedWebNameList(%v) = %q; want %q", tc.groups, got, tc.want)
			}
		})
	}
}

func TestFlattenWebGroupParentCodesDedupsAcrossGroups(t *testing.T) {
	groups := []WebGroupMatch{
		{WebName: "FF901", ParentCodes: []string{"SP458484", "SP001187"}},
		{WebName: "FF901 Carbon", ParentCodes: []string{"SP458516"}},
		{WebName: "Other", ParentCodes: []string{"SP458484"}}, // duplicate
	}

	got := FlattenWebGroupParentCodes(groups)
	want := []string{"SP458484", "SP001187", "SP458516"}

	if !reflect.DeepEqual(got, want) {
		t.Errorf("FlattenWebGroupParentCodes = %#v; want %#v", got, want)
	}
}
