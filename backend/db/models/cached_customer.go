...
type CachedCustomer struct {
	...
	// REGION (Miền) is enriched on each daily sync by reading the external
	// reference Postgres (cloudify.cloudify_customers) by MA. The cache job only
	// reads from Postgres — it never writes to it.
	REGION          string    `gorm:"type:varchar(255);index:idx_cc_region" json:"region"`
