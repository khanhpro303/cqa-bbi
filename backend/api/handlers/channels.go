...
// testTemplateVariant returns the (endpointPath, payload) for one of the
// brute-force shapes used by SendTestTemplateMessage. Variant 0 means "stick
// with the default shape we believe is correct"; higher numbers test specific
// hypotheses one knob at a time so the failing knob can be isolated.
func testTemplateVariant(variant int, zaloUserID, imageURL string) (string, map[string]interface{}, string) {
	const headerText = "🔧 Test rich message — chọn 1 option."

	// Three demo options used by most variants.
	type opt struct{ title, subtitle, payload string }
	opts := []opt{
		{"Option A", "Gửi postback #test_template:a", "#test_template:a"},
		{"Option B", "Gửi postback #test_template:b", "#test_template:b"},
		{"Option C", "Gửi postback #test_template:c", "#test_template:c"},
	}

	elementsHide := func(withImage bool) []map[string]interface{} {
		els := make([]map[string]interface{}, 0, len(opts))
		for _, o := range opts {
			el := map[string]interface{}{
				"title":    o.title,
				"subtitle": o.subtitle,
				"buttons": []map[string]interface{}{
					{"title": o.title, "type": "oa.query.hide", "payload": o.payload},
				},
			}
			if withImage {
	...
	elementsShow := func(withImage bool) []map[string]interface{} {
		els := make([]map[string]interface{}, 0, len(opts))
		for _, o := range opts {
			el := map[string]interface{}{
				"title":    o.title,
				"subtitle": o.subtitle,
				"buttons": []map[string]interface{}{
					{"title": "Chọn " + o.title, "type": "oa.query.show", "payload": o.payload},
				},
			}
			if withImage {
				el["image_url"] = imageURL
	...
	switch variant {
	case 1: // doc-verbatim: 2 elements like the user pasted, show, no image, with message.text
		return "/message/cs", map[string]interface{}{
			"recipient": map[string]interface{}{"user_id": zaloUserID},
			"message": map[string]interface{}{
				"text": "🤖 AI đã hiểu yêu cầu. Bạn muốn tiếp tục tư vấn về nội dung nào dưới đây?",
				"attachment": map[string]interface{}{
					...
					"payload": map[string]interface{}{
						...
						"elements": []map[string]interface{}{
							{
								"title":    "Kịch bản 1: Tư vấn báo giá",
								"subtitle": "Xem bảng giá dịch vụ mới nhất",
								"buttons": []map[string]interface{}{
									{"title": "Chọn Báo Giá", "type": "oa.query.show", "payload": "KICH_BAN_BAO_GIA"},
								},
							},
							{
								"title":    "Kịch bản 2: Hỗ trợ kỹ thuật",
								"subtitle": "Gặp chuyên gia xử lý lỗi",
								"buttons": []map[string]interface{}{
									{"title": "Chọn Hỗ Trợ", "type": "oa.query.show", "payload": "KICH_BAN_KY_THUAT"},
								},
							},
						},
					},
		...
		}, "doc-verbatim: show buttons, no image, message.text, /cs"

	case 2: // 3 demo options, show, no image, with message.text
		return "/message/cs", map[string]interface{}{
			"recipient": map[string]interface{}{"user_id": zaloUserID},
			"message": map[string]interface{}{
				"text": headerText,
				"attachment": map[string]interface{}{
					...
					"payload": map[string]interface{}{
						"template_type": "list",
						"elements":      elementsShow(false),
					},
				},
			},
		}, "3 demo opts, show, no image, message.text, /cs"

	case 3: // like 2 but WITH image_url
		return "/message/cs", map[string]interface{}{
			"recipient": map[string]interface{}{"user_id": zaloUserID},
			"message": map[string]interface{}{
				"text": headerText,
				"attachment": map[string]interface{}{
					...
					"payload": map[string]interface{}{
						"template_type": "list",
						"elements":      elementsShow(true),
					},
				},
			},
		}, "3 demo opts, show, WITH image, message.text, /cs"

	case 4: // like 2 but hide instead of show
		return "/message/cs", map[string]interface{}{
			"recipient": map[string]interface{}{"user_id": zaloUserID},
			"message": map[string]interface{}{
				"text": headerText,
				"attachment": map[string]interface{}{
					...
					"payload": map[string]interface{}{
						"template_type": "list",
						"elements":      elementsHide(false),
					},
				},
			},
		}, "3 demo opts, HIDE, no image, message.text, /cs"

	case 5: // attachment-only, drop message.text
		return "/message/cs", map[string]interface{}{
			"recipient": map[string]interface{}{"user_id": zaloUserID},
			"message": map[string]interface{}{
				"attachment": map[string]interface{}{
		...
		}, "3 demo opts, show, no image, NO message.text, /cs"

	case 6: // sanity: plain text only
		return "/message/cs", map[string]interface{}{
			"recipient": map[string]interface{}{"user_id": zaloUserID},
			"message": map[string]interface{}{
				"text": "🔧 Plain text sanity check from test-template handler.",
			},
		}, "sanity: text only, /cs"

	case 7: // try /transaction endpoint instead of /cs
		return "/message/transaction", map[string]interface{}{
			"recipient": map[string]interface{}{"user_id": zaloUserID},
			"message": map[string]interface{}{
				"text": headerText,
				"attachment": map[string]interface{}{
		...
		}, "3 demo opts, show, no image, message.text, /transaction"

	default: // 0 — our current default: 3 opts, hide, image, message.text
		return "/message/cs", map[string]interface{}{
			"recipient": map[string]interface{}{"user_id": zaloUserID},
			"message": map[string]interface{}{
				"text": headerText,
				"attachment": map[string]interface{}{
					...
					"payload": map[string]interface{}{
						"template_type": "list",
						"elements":      elementsHide(true),
					},
				},
			},
		}, "default: 3 demo opts, hide, image, message.text, /cs"
	}
}

// SendTestTemplateMessage sends a 3-button "list" template via Zalo OA V3 to
// a whitelisted staff Zalo user for owner-side smoke testing. Restricted to
// role=owner via router middleware.
//
// Accepts an optional "variant" field in the JSON body (0-7) to brute-force
// different payload shapes; the response includes the exact JSON sent and the
// full Zalo response so the failing knob can be isolated. See
// testTemplateVariant for the shape of each variant.
func SendTestTemplateMessage(c *gin.Context) {
	...
	endpointPath, payload, variantDesc := testTemplateVariant(req.Variant, req.ZaloUserID, imageURL)

	// Log the exact outgoing payload so it can be diffed against Zalo's spec.
	payloadJSON, _ := json.Marshal(payload)
	log.Printf("[test-template] variant=%d desc=%q endpoint=%s payload=%s",
		req.Variant, variantDesc, endpointPath, string(payloadJSON))
	...
	zaloResp, sendErr := adapter.SendRawToZaloEndpoint(ctx, endpointPath, payload)
	...
	if sendErr != nil {
		...
		resp["details"] = sendErr.Error()
		log.Printf("[error] test-template variant=%d send failed: tenant=%s channel=%s actor=%s target=%s err=%v",
			req.Variant, tenantID, channelID, actorUserID, req.ZaloUserID, sendErr)
		c.JSON(http.StatusBadGateway, resp)
		return
	}

	resp["sent"] = true
	log.Printf("[audit] test-template variant=%d sent: tenant=%s channel=%s actor=%s target=%s",
		req.Variant, tenantID, channelID, actorUserID, req.ZaloUserID)
	c.JSON(http.StatusOK, resp)
}
