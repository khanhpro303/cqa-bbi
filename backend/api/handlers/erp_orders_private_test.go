...
func TestScopeApprovedOrdersCodes(t *testing.T) {
	...
	tests := []struct {
		...
		{"all preserves label", []string{"S001_1 - Huy"}, "all", nil, nil, []string{"S001_1 - Huy"}},
		...
		{"assigned case-insensitive", []string{"s052"}, "assigned", nil, allowed, []string{"s052"}},
		{"own matches own", []string{"S001"}, "own", []string{"S001"}, nil, []string{"S001"}},
		{"own rejects other", []string{"S002"}, "own", []string{"S001"}, nil, []string{}},
		{"own multi keeps both shops", []string{"S001", "S002"}, "own", []string{"S001", "S002"}, nil, []string{"S001", "S002"}},
		{"unknown scope denies", []string{"S001"}, "weird", nil, allowed, []string{}},
		{"empty resolved", nil, "all", nil, nil, []string{}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := scopeApprovedOrdersCodes(tt.resolved, tt.scopeType, tt.ownCodes, tt.allowed)
			if len(got) == 0 && len(tt.want) == 0 {
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
