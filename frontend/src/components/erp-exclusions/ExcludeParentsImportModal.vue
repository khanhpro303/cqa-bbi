  ...
  <v-dialog v-model="dialog" max-width="560" persistent>
    <v-card>
      <v-card-title class="d-flex align-center">
        ...
        Import Excel danh sách loại trừ
      </v-card-title>
      <v-card-text>
        <p class="text-body-2 mb-3">
          File Excel cần đúng 2 cột:
          <strong>cột A = SKU cha</strong>,
        ...
        >
          Tải file mẫu (đã pre-fill SKU cha hiện có)
        </v-btn>
        ...
        >
          ...
          Cache AI hiện có <strong>{{ result.cache_rows }}</strong> sản phẩm.
        </v-alert>
...
async function downloadTemplate(): Promise<void> {
  ...
  try {
    const response = await api.get(
      `/tenants/${props.tenantId}/erp/parent-skus/template`,
      { responseType: 'blob' },
    )
    const url = URL.createObjectURL(new Blob([response.data]))
    const a = document.createElement('a')
    a.href = url
    a.download = `parent-sku-template-${Date.now()}.xlsx`
...
async function upload(): Promise<void> {
  ...
  try {
    ...
    const { data } = await api.post<ImportResult>(
      `/tenants/${props.tenantId}/erp/parent-skus/import`,
      form,
