...
function assertCompiles(messages: Record<string, unknown>, locale: string): void {
  for (const [key, value] of Object.entries(messages)) {
    if (typeof value !== 'string') {
      assertCompiles(value as Record<string, unknown>, `${locale}.${key}`)
      continue
    }
    expect(
...
describe('i18n messages compile under the strict (production) vue-i18n compiler', () => {
  it('every vi message compiles', () => assertCompiles(vi as Record<string, unknown>, 'vi'))
  it('every en message compiles', () => assertCompiles(en as Record<string, unknown>, 'en'))
})
